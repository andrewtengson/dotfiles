...
function renderAmpUserMessage(
  ...
  return ["", ...body.map((line) => styledUserLine(line, width, theme, color))];
}
...
function patchUserMessageRender(
  ...
  prototype.render = function renderWithAmpUserMessage(
    width: number,
  ): string[] {
    const original =
      prototype.__ampUserMessageOriginalRender ?? prototype.render;
    ...
    const thinkingLevel =
      prototype.__ampUserMessageGetThinkingLevel?.() ?? "off";
    const color = getThinkingColor(thinkingLevel);
    const ampLines = renderAmpUserMessage(
