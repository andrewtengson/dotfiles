...
function awsConfig(profile: string, key: string): string | undefined {
	try {
		return execFileSync("aws", ["configure", "get", key, "--profile", profile], {
			encoding: "utf-8",
...
function getCacheKey(profile: string): string | undefined {
	return awsConfig(profile, "sso_session") ?? awsConfig(profile, "sso_start_url");
}
...
export default async function (pi: ExtensionAPI) {
	...
	async function runLogin(ctx?: ExtensionContext): Promise<void> {
		ctx?.ui.notify(`AWS SSO expired for ${profile}. Opening browser...`, "warning");
		await pi.exec("aws", ["sso", "login", "--profile", profile], { timeout: LOGIN_TIMEOUT_MS });
